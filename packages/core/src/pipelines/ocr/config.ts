...
export interface PipelineModelSelection {
  docOrientationModelName: string | null;
  docUnwarpingModelName: string | null;
  textDetectionModelName: string | null;
  textLineOrientationModelName: string | null;
...
export function normalizeOcrPipelineConfig(input: unknown): NormalizedPipelineConfig {
  ...
  const docPreprocessor = isPlainObject(subPipelines?.DocPreprocessor)
    ...
    : null;
  const docPreprocessorSubModules = isPlainObject(docPreprocessor?.SubModules)
    ? docPreprocessor.SubModules
    : {};
  const docOrientation = isPlainObject(docPreprocessorSubModules.DocOrientationClassify)
    ? docPreprocessorSubModules.DocOrientationClassify
    : null;
  const docUnwarping = isPlainObject(docPreprocessorSubModules.DocUnwarping)
    ? docPreprocessorSubModules.DocUnwarping
    : null;
  const useDocOrientationClassify = Boolean(docPreprocessor?.use_doc_orientation_classify);
  const useDocUnwarping = Boolean(docPreprocessor?.use_doc_unwarping);
  ...
  const textLineOriAsset = getModuleAsset(
    "textLineOri",
    "SubModules.TextLineOrientation",
    textLineOrientation
  );
  const docOriAsset = getModuleAsset(
    "docOri",
    "SubPipelines.DocPreprocessor.SubModules.DocOrientationClassify",
    docOrientation
  );
  const docUnwarpAsset = getModuleAsset(
    "docUnwarp",
    "SubPipelines.DocPreprocessor.SubModules.DocUnwarping",
    docUnwarping
  );
  ...
  return {
    ...
    unsupportedFeatures: [],
    modelSelection: {
      docOrientationModelName: getModuleModelName(docOrientation),
      docUnwarpingModelName: getModuleModelName(docUnwarping),
      textDetectionModelName: getModuleModelName(textDetection),
      textLineOrientationModelName: getModuleModelName(textLineOrientation),
      textRecognitionModelName: getModuleModelName(textRecognition)
    },
    assets: {
      ...(docOriAsset ? { docOri: docOriAsset } : {}),
      ...(docUnwarpAsset ? { docUnwarp: docUnwarpAsset } : {}),
      ...(detAsset ? { det: detAsset } : {}),
      ...(textLineOriAsset ? { textLineOri: textLineOriAsset } : {}),
    ...
    useDocOrientationClassify: useDocPreprocessor && useDocOrientationClassify,
    useDocUnwarping: useDocPreprocessor && useDocUnwarping,
    useTextLineOrientation: useTextlineOrientation,
    pipelineBatchSize,
    textDetectionBatchSize,
    textLineOrientationBatchSize: batchSizeOrOne(textLineOrientation?.batch_size),
