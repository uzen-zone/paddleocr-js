...
1. runtime initialization (OpenCV.js + ONNX Runtime Web)
2. execution backend selection (WASM / WebGPU / auto)
3. model download (detection, recognition, and optional preprocessing models)
4. inference session creation
5. OCR pipeline execution

## OCR Pipeline

The OCR pipeline processes images through the following stages:

```
Input Image
  │
  ├─ [Optional] Document Orientation Classification
  │    Classifies the overall document orientation (0°/90°/180°/270°)
  │    and rotates the image upright automatically.
  │
  ├─ [Optional] Document Unwarping / Dewarping
  │    Corrects perspective distortion and page curl with a learnable
  │    warping model, producing a flat document image.
  │
  ├─ Text Detection
  │    Detects text regions and outputs bounding polygons.
  │
  ├─ [Optional] Text Line Orientation Classification
  │    Classifies each cropped text line (0°/180°) and rotates
  │    upside-down lines before recognition.
  │
  └─ Text Recognition
       Recognizes characters from each oriented text line.
```

Each optional stage can be independently enabled or disabled via construction options. When enabled, the corresponding model asset is downloaded, validated, and loaded during initialization.

### Result structure

When preprocessing stages are enabled, the OCR result includes additional fields:

- `OcrResultItem.textLineOrientation`: per-text-line orientation result (angle + score)
- `OcrResult.preprocessing.docOrientation`: document-level orientation result
- `OcrResult.preprocessing.docUnwarping`: document unwarping result metadata
