...
`@uzen/paddleocr-js` 是在前端运行 PaddleOCR 产线的浏览器 SDK 包。
...
也可设置 `textLineOrientationBatchSize` 控制文本行方向分类的批处理大小（默认: 6）。
...
- `preprocessing`（可选）：启用文档方向或去歪曲时出现
...
if (result.preprocessing?.docOrientation) {
  console.log(`图像旋转了 ${result.preprocessing.docOrientation.angle}°`);
}

// 逐行文本方向
for (const item of result.items) {
  if (item.textLineOrientation) {
    console.log(`文本 "${item.text}" 方向: ${item.textLineOrientation.angle}°`);
  }
}
```
