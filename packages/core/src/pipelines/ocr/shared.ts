...
const PP_OCRV6_LANG_VERSION_MODEL_SELECTION: Readonly<PipelineModelSelection> = Object.freeze({
  ...
  docUnwarpingModelName: DEFAULT_MODEL_SELECTION.docUnwarpingModelName,
  textDetectionModelName: "PP-OCRv6_small_det",
  textLineOrientationModelName: DEFAULT_MODEL_SELECTION.textLineOrientationModelName,
...
const OCR_MODEL_ROLES: Readonly<ModelRole[]> = Object.freeze([
  {
    assetKey: "docOri",
    modelRole: "DocOrientationClassify",
    selectionKey: "docOrientationModelName",
    ...
    nameLabel: "document orientation model name",
    assetLabel: "document orientation model asset",
    assetRequirementError: "doc_orientation_model_dir requires doc_orientation_model_name."
  },
  {
    assetKey: "docUnwarp",
  ...
  {
    assetKey: "textLineOri",
    modelRole: "TextLineOrientation",
    selectionKey: "textLineOrientationModelName",
...
function readExplicitBatchSizes(options: Record<string, unknown>): {
  ...
  return {
    ...
    textLineOri: toBatchSizeOption(
      readAliasedOption(
        options,
        ["textLineOrientationBatchSize", "textline_orientation_batch_size"],
...
function mergeNormalizedPipelineConfigWithExplicit(
  ...
  if (explicitBatch.textLineOri !== undefined) {
    merged.textLineOrientationBatchSize = explicitBatch.textLineOri;
  }
...
function createOcrAssets(
  ...
  if (Object.values(assets).some((asset) => !asset)) {
    if (!assets.det || !assets.rec) {
      throw new Error("OCR model selection must define both detection and recognition models.");
    }
  }

  return Object.fromEntries(
    Object.entries(assets).filter((entry): entry is [string, ModelAsset] => Boolean(entry[1]))
  );
}
